...
class SfdxTasks {
    ...
    static async enqueueApexTests(usernameOrAlias, sfdxEntities, shouldSkipCodeCoverage = false) {
        ...
        fs_1.writeSync(stream, `ApexClassId,ShouldSkipCodeCoverage${constants_1.default.EOL}`);
        for (const sfdxEntity of sfdxEntities) {
            fs_1.writeSync(stream, `${sfdxEntity.id},${shouldSkipCodeCoverage}${constants_1.default.EOL}`);
        }
