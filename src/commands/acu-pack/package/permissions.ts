...
export default class Permissions extends CommandBase {
  ...
  protected static flagsConfig = {
    ...
    metadata: flags.string({
      ...
      description: CommandBase.messages.getMessage('package.permissions.metadataFlagDescription', [
        SfdxPermission.defaultPermissionMetaTypes.join(', '),
      ]),
    }),
  ...
  protected async runInternal(): Promise<void> {
    ...
    const metaNames = Utils.sortArray(
      this.flags.metadata ? this.flags.metadata.split() : SfdxPermission.defaultPermissionMetaTypes
    );
    this.metaNames = new Set(metaNames);

    // Are we including namespaces?
    this.namespaces = this.flags.namespaces ? new Set<string>(this.flags.namespaces.split()) : new Set<string>();
